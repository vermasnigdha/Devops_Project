...
<h1 style="margin: auto; width: 80%; text-align: center;">View Breed</h1>
...
<table class="table table-striped" style="margin: auto; width: 80%; text-align: center;">
	...
	<tbody>
		...
		<c:forEach items="${AllBreed}" var="x">
		
		<tr>
			<td>${x.getId()}</td>
			<td>${x.getName()}</td>
			<td>${x.getAverage_weight()}</td>
			<td>${x.getPrice()}</td>
			<td><img src="${x.getImagePath()}" style="height: 50px; width: 50px;"></td>
			<td><a href="${pageContext.request.contextPath}/UpdateBreed/${x.getId()}" class="btn btn-danger">Update</a></td>
			<td><a href="${pageContext.request.contextPath}/DeleteBreedFromDB/${x.getId()}" class="btn btn-danger">Delete</a></td>
		</tr>
